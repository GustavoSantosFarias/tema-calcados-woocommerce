    ...
    public function data(){
		...
		while ($data_row = fgetcsv($handle,1000, ",")) {
            ...
            for ($i=0; $i < count($header) - 2; $i++) { 
                ...
                if(!isset($this->structure[$data_row[3]])){
                    $this->structure[$data_row[3]] = array();
                }

                if(!isset($this->structure[$data_row[3]][$index])){
					$this->structure[$data_row[3]][$index] = array();
				}

                if($data_row[$i] !== ""){
                    array_push($this->structure[$data_row[3]][$index],$data_row[$i]);
                }
            }	
